...
const TambahKepalaRT = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const formData = {
      ...
      wilayahRTId: selectedRT,
    };

    try {
      const response = await axios.post(
        "http://localhost:2001/api/register/rt",
        formData,
        authConfig
      );
      Swal.fire({
      ...
      <div className="block w-full">
        <section className="bg-gray-300  w-full">
          <div className="mx-auto max-w-screen-xl px-4 py-16 sm:px-6 lg:px-8">
            <nav className="text-xl text-center font-semibold mb-8 sticky top-0 bg-gray-300 z-10 m-30">
              Tambah Kepala RT
            </nav>
