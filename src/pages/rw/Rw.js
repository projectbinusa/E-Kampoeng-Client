  ...
  const getAll = async (page) => {
    try {
      const response = await axios.get(
        `http://localhost:2001/e-kampoeng/api/rw?pageNumber=${page}&pageSize=${size}`,
        authConfig
      );
      setPages(response.data.data.totalPages);
      setRW(response.data.data.content);
    } catch (error) {
  ...
  const Delete = async (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          await axios.delete(
            "http://localhost:2001/e-kampoeng/api/rt/" + id,
            authConfig
          );
          Swal.fire({
            title: "Terhapus!",
